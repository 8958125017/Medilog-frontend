...
user : any ;
  constructor(public globalService:GlobalServiceService,
    ...
    private http: Http) { 
        var data = localStorage.getItem('pharmacy');        
        if(data) {
...
sendRequest(){  
            	if(this.billForm.value.filename){
            		   this.loading=true;
            		   let pharmacyId = this.user._id;
            		   this.billForm.value.pharmacyId = pharmacyId;
            		   this.billForm.value.aadharno = this.billForm.value.aadharno.toString();
            		   let postData = this.billForm.value;		   
            		   const url=this.globalService.basePath+'pharmacy/uploadBillBypharmacy';
